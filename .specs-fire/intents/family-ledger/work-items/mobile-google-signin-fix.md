...
- [x] Proactive mitigation for the leading theory (in-app browser blocking), applied without waiting for device-confirmed root cause since real device access still isn't available: `lib/in-app-browser.ts` detects known in-app-browser user-agent signatures (Facebook, Instagram, LINE, WeChat, TikTok, Twitter/X, generic Android `wv` WebView marker) and `components/InAppBrowserWarning.tsx` shows a banner on `/signin` with "Open in Browser" instructions plus a copy-link fallback, before the user even attempts Google sign-in. 6 unit tests (real browser UAs correctly pass through, known in-app UAs correctly flagged). This directly serves the family-chat-link-sharing use case flagged in Technical Notes below.
- [ ] Still open — genuinely requires the reporting user's own device: confirm whether the in-app-browser theory was actually the cause (the new banner will make this self-evident next time it happens — if they see the banner, that was it) — if they *don't* see the banner and still see nothing happen, the remaining theories (client-side JS/event issue, or OAuth completes but the session cookie doesn't stick — check `__Secure-` cookie behavior under mobile Safari ITP if on iOS) still need real-device debugging
- [ ] Once confirmed one way or the other, close this out with the actual result
